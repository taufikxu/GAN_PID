...
def get_fid(
    imgs,
    stat_file="/home/kunxu/Workspace/0Baseline/sngan_projection/datasets/cifar-10-fid.npz.bak"
):
    path = "/home/kunxu/Workspace/0Baseline/sngan_projection/datasets/inception_model"
    # stat_file = "/home/kunxu/Workspace/0Baseline/sngan_projection/datasets/cifar-10-fid.npz"
    ...
    if os.path.exists(stat_file) is False:
        np.savez(stat_file, mu=mean, sigma=cov)
...
def inception_score(model, ims, batch_size=100, splits=10):
    ...
    print('batch_size:{}, n_ims{}, n_batches{}'.format(batch_size, n,
                                                       n_batches))
...
def evaluation(ims):
    model = load_inception_model(None)
    ims = np.asarray(np.clip(ims * 127.5 + 127.5, 0.0, 255.0), dtype=np.uint8)
    ims = ims.astype(np.float32)
    mean, std = inception_score(model, ims, splits=10)
    print(mean, std)
    return mean, std
